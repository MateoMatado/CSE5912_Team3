...
public class IslandGenTest : MonoBehaviour, IData
{
    ...
    void Start()
    {
        centerPoint.position = new Vector3(Width / 2f, 200, Length / 2f);
        StartGen(null, null);
        // EventsPublisher.Instance.SubscribeToEvent("PlayerJump", StartGen);
    }

    private void StartGen(object sender, object data)
    {
        ...
        DIslands.Add(Islands[7].name.ToString(), new Vector2(800, 800));
        if (Island.Count != 0)
        {
    ...
    private IEnumerator GenerateIslands()
    {
        EIslandArray = new int[Width, Length];
        gameObjectIslands = new GameObject[Width, Length];
        int BiasDec = Bias;

        for (int x = 0; x < DIslands["Nav2IslandBasic"].x; x++)
        {
            for (int y = 0; y < DIslands["Nav2IslandBasic"].y; y++)
            {
    ...
    private IEnumerator GenerateChains()
    {
        Debug.Log("Attempt to generate chains");
        ...
        foreach(GameObject Island in IslandList)
        {
            ...
            {
                if(Random.Range(0,100) <= 1 && num < ChainNum)
                {
